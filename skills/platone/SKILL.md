...
Deposita il concetto nel Third Brain tramite `tb save` rispettando i **Vincoli di Purezza Semantica**.
...
3. **Verifica di Conformità**: Prima di ogni `tb save`, chiediti: *"Se eliminassi l'intero storico di questa sessione, un estraneo capirebbe il valore di questa nota senza chiedersi chi l'ha detta o cosa significhi il gergo usato?"* $\to$ Se la risposta è NO, riscrivila.
4. **Salva e Proponi**: Chiama `tb save` per ogni concetto e presenta le perle selezionate in chat.
