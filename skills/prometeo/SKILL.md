...
The queries must be realistic and something a pi or Claude Code user would actually type. Not abstract requests, but requests that are concrete and specific and have a good amount of detail. For instance, file paths, personal context about the user's job or situation, column names and values, company names, URLs. A little bit of backstory. Some might be in lowercase or contain abbreviations or typos or casual speech. Use a mix of different lengths, and focus on edge cases rather than making them clear-cut (the user will get a chance to sign off on them).
...
Understanding the triggering mechanism helps design better eval queries. In pi, skills appear in the system prompt as XML with their name + description. The model decides whether to read the full SKILL.md based on that description — triggering is detected when the model calls `read` on the SKILL.md file. The model only does this for tasks complex enough to warrant consulting the skill; simple one-step queries may not trigger even if the description matches.
